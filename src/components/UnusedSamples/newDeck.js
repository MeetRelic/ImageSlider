...
class Deck extends Component {
    ...
    componentDidMount(){
        ...
        for(let i = 0; i <= this.state.numberof_crdsby_index ; i++){
            new_cards.push(
                <Card picsum= {`https://picsum.photos/600/${350 + i}`} key={i} myid={`card_${i}`} />
            )
        }
        this.setState({cards:new_cards},()=> {
            ...
            this.nav_buttons_placement_as_percentage = 60;
            this.nav_buttons_placement_as_percentage = window.innerWidth < 700 ? 100 : this.nav_buttons_placement_as_percentage;
            ...
            this.nav_buttons_container.style.width = `${this.nav_buttons_placement_as_percentage}vw`;
            ...
            window.addEventListener('resize' , () => {
                ...
                this.order_cards();
                  ...
                  this.right_boundary = parseFloat(this.images.children[this.state.numberof_crdsby_index].style.left) + this.new_width;
    ...
    order_cards = ()=>{
        ...
        middlecardbyindex = Math.floor(this.state.numberof_crdsby_index /2);
        
        let counter_for_right = 1,
            counter_for_left = middlecardbyindex;
    ...
    handle_boundaries = () =>{
        if(this.last_positions[this.state.numberof_crdsby_index] >= this.right_boundary){
            const begining_of_deck = this.last_positions[0] - this.new_width;;
            this.images.children[this.state.numberof_crdsby_index].style.left = `${begining_of_deck}px`;
            this.last_positions[this.state.numberof_crdsby_index] = begining_of_deck;

            this.images.insertBefore(this.images.children[this.state.numberof_crdsby_index] ,  this.images.children[0]);
            this.last_positions.splice(0,0,this.last_positions.pop());
           }

           if(this.last_positions[0] <= this.left_boundary){
            const end_of_deck = this.last_positions[this.state.numberof_crdsby_index] + this.new_width;;
            this.images.children[0].style.left = `${end_of_deck}px`;
            this.last_positions[0] = end_of_deck;

            this.images.appendChild(this.images.children[0] ,  this.images.children[this.state.numberof_crdsby_index]);
            this.last_positions.splice(this.state.numberof_crdsby_index,0,this.last_positions.shift());
           
           }
    }
    
             /*--------------- SNAP BACK LOGIC Skeleton -------------------*/
            snap_back = () =>{
            ...
            /*----------------------------------------------------- */
         /*
             <img style = {styles.nav_buttons} ref= {ref_id => this.nav_button_prev = ref_id} src="./left.png" alt="prev" className="nav_buttons" id="prev" onClick={this.handle_prev} />
               
                    <img style = {styles.nav_buttons} ref= {ref_id => this.nav_button_next = ref_id} src="./right.png" alt="next" className="nav_buttons" id="next" onClick={this.handle_next} />
               
         */   
/*
 
                <div style = {styles.nav_buttons_container} ref= {ref_id => this.nav_buttons_container = ref_id} className="nav_buttons_container">
                 <button style = {styles.nav_buttons} ref= {ref_id => this.nav_button_prev = ref_id} onClick = {this.handle_prev} id= "prev">Prev</button>
                 <button style = {styles.nav_buttons} ref= {ref_id => this.nav_button_next = ref_id}  onClick = {this.handle_next} id= "next">Next</button>
                 </div>
             
    <div style = {styles.nav_buttons_container} ref= {ref_id => this.nav_buttons_container = ref_id} className="nav_buttons_container">
  <img style = {styles.nav_buttons} ref= {ref_id => this.nav_button_prev = ref_id} src="./left.png" alt="prev" className="nav_buttons" onClick={this.handle_prev} id="prev"  />
               
  <img style = {styles.nav_buttons} ref= {ref_id => this.nav_button_next = ref_id} src="./right.png" alt="next" className="nav_buttons" onClick={this.handle_next} id="next"  />
               </div>
